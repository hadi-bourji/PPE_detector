...
class Mosaic(nn.Module):
    ...
    def forward(self, img, labels, img_paths, output_size=640):
        ...
        # images must be of the same size, so we will resize them to the output_size
        for i in indices:
            new_img = cv2.imread(img_paths[i])
            ...
            lbl_path = img_paths[i].replace('/images/', '/labels/').rsplit('.', 1)[0] + '.txt'
